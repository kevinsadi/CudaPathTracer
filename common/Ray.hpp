...
struct Ray
{
    ...
    bool shadowTest; // true: the ray is used for shadow test and if hitDistance < shadowDistance shadow is detected
    double shadowDistance; // the time of the shadow ray
    // double t; // transportation time,
    // double t_min, t_max;
    FUNC_QUALIFIER inline Ray() {}
    FUNC_QUALIFIER inline Ray(const glm::vec3& ori, const glm::vec3& dir, const bool _shadowTest = false, const double _shadowDistance = kDoubleInfinity) : origin(ori), direction(dir), shadowTest(_shadowTest), shadowDistance(_shadowDistance)
    {
